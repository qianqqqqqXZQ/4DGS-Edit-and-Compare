...
function createInfiniteGrid(){const geometry=new THREE.PlaneBufferGeometry(10000,10000);const material=new THREE.ShaderMaterial({uniforms:{gridColor:{value:new THREE.Color(0x1b2940)}},vertexShader:'varying vec2 vGridPosition;void main(){vGridPosition=position.xy;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}',fragmentShader:'varying vec2 vGridPosition;uniform vec3 gridColor;void main(){vec2 coordinate=vGridPosition;vec2 derivative=max(fwidth(coordinate),vec2(0.0001));vec2 distanceToLine=abs(fract(coordinate-0.5)-0.5);vec2 line=distanceToLine/derivative;float intensity=1.0-min(min(line.x,line.y),1.0);if(intensity<.01)discard;gl_FragColor=vec4(gridColor,intensity*.78);}',extensions:{derivatives:true},side:THREE.DoubleSide,transparent:true,depthTest:true,depthWrite:false,polygonOffset:true,polygonOffsetFactor:1,polygonOffsetUnits:1});const grid=new THREE.Mesh(geometry,material);grid.renderOrder=-10;grid.userData.infiniteGrid=true;grid.userData.lastSnapX=NaN;grid.userData.lastSnapY=NaN;return grid}
function createPointMaterial(size){const material=new THREE.PointsMaterial({size:Number(size)||Number($('pointSize').value)||3,vertexColors:true,sizeAttenuation:false,transparent:true,depthTest:true,depthWrite:true,alphaTest:.001});material.extensions=material.extensions||{};material.extensions.derivatives=true;material.onBeforeCompile=shader=>{shader.fragmentShader=shader.fragmentShader.replace('#include <alphatest_fragment>','float pointDistance=length(gl_PointCoord-vec2(0.5));float pointAA=fwidth(pointDistance);float pointAlpha=1.0-smoothstep(0.5-pointAA,0.5+pointAA,pointDistance);diffuseColor.a*=pointAlpha;if(diffuseColor.a<=0.001)discard;\n#include <alphatest_fragment>')};material.customProgramCacheKey=()=> 'rounded-point-sprite-depth-v2';return material}
function syncInfiniteGrid(targetScene,target){const grid=targetScene&&targetScene.userData.infiniteGrid;if(!grid||!target)return;const snap=1000,x=Math.round(target.x/snap)*snap,y=Math.round(target.y/snap)*snap;if(grid.userData.lastSnapX===x&&grid.userData.lastSnapY===y)return;grid.position.set(x,y,0);grid.userData.lastSnapX=x;grid.userData.lastSnapY=y}
